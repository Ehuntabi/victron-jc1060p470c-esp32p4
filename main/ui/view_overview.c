...
ui_device_view_t *ui_overview_view_create(ui_state_t *ui, lv_obj_t *parent)
{
    ...
    lv_obj_set_style_pad_bottom(ov->card_bat, UI_PAD_CARD - 10, 0);
    {
        ...
        if (bat_img) {
            /* margin_top negativo: participa en el layout flex y desplaza
             * el objeto verticalmente (translate_y solo afectaba al render
             * y se quedaba clipped al borde del card). */
            lv_obj_set_style_margin_top(bat_img, -25, 0);
        }
    }
