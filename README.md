...
- **Batería**: 24 h en PSRAM (~945 KB) con muestreo cada 10 s y `avg/max/min` por intervalo. CSV diario en `/sdcard/bateria/YYYY-MM-DD.csv` (5 columnas).
...
- **Battery**: 24 h in PSRAM (~945 KB) sampled every 10 s with avg/max/min per bin. Daily CSV at `/sdcard/bateria/YYYY-MM-DD.csv` (5 columns).
