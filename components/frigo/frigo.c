...
{
    if (t_aletas < (float)(t_min - 5)) return 0;
    if (t_aletas > (float)t_max)       return 100;
    return 50;
}
...
static void frigo_task(void *arg)
{
    while (1) {
        ...
        /* Disparar conversión en todos los sensores */
        for (int i = 0; i < s_state.n_sensors; i++) {
            ds18b20_trigger_temperature_conversion(s_devs[i]);
        }
        vTaskDelay(pdMS_TO_TICKS(800)); /* esperar conversión */
        ...
        /* Asignar según configuración — filtrar valores inválidos DS18B20 */
        ...
        /* PWM ventilador */
        uint8_t pct = compute_fan(s_state.T_Aletas, s_state.T_min, s_state.T_max);
...
esp_err_t frigo_init(frigo_update_cb_t cb)
{
    ...
    while (onewire_device_iter_get_next(iter, &dev) == ESP_OK
        ...
        if (ds18b20_new_device_from_enumeration(&dev, &ds_cfg, &s_devs[s_state.n_sensors]) == ESP_OK) {
    ...
    /* Tarea de lectura */
    xTaskCreate(frigo_task, "frigo", 4096, NULL, 5, NULL);
    return ESP_OK;
}
