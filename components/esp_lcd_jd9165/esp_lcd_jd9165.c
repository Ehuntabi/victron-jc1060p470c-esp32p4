...
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel)
{
    ...
    /* FORK LOCAL — read ID deshabilitado.
     *
     * El upstream lee el Display ID con cmd 0x04 (rx_param). Internamente
     * llama a mipi_dsi_hal_host_gen_read_short_packet, que hace busy-wait
     * SIN timeout esperando el FIFO. Si el panel no responde a tiempo
     * (variación entre lotes, timing post power-on), el host se cuelga 5 s
     * y dispara WDT IDLE0. El Guition JC1060P470C es uno de los afectados.
     *
     * El ID solo se usaba para un log de debug, no afecta a la operación
     * del panel. Bug equivalente: esp-idf issue #15137. */
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        // Send command
        ESP_LOGD(TAG, "init[%d/%d] cmd=0x%02X bytes=%d delay=%dms",
