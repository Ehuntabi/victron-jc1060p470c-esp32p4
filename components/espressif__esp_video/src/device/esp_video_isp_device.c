...
/* PARCHE LOCAL: gamma NEUTRA (identidad y = x). No es un valor inventado: es la
 * curva que no altera nada, la referencia con la que comprobar si la tuberia del
 * ISP entrega imagen. Si con esto sale foto (aunque plana), lo que falta es solo
 * afinar; si sigue saliendo ruido, el problema no es de calibracion. */
static uint32_t joint_gamma_lineal(uint32_t x)
{
    return x;
}
...
esp_err_t esp_video_isp_start_by_csi(const esp_video_csi_state_t *state, const struct v4l2_format *format)
{
    ...
    if (state->bypass_isp) {
        ...
        /* ── PARCHE LOCAL (joint/victron) ─────────────────────────────────────
         * El original solo CREA y habilita el procesador: los modulos de
         * revelado quedan apagados y la salida son datos practicamente crudos
         * (ruido con rayas). Con un sensor soportado de fabrica los enciende el
         * controlador IPA a partir de su perfil JSON, pero el OV02C10 no tiene
         * perfil publicado. Encenderlos despues, desde codigo de usuario por los
         * controles V4L2, mata la tuberia (probado). Este es el unico instante
         * valido: el ISP ya existe y el stream todavia no entrega.
         * Solo el demosaico: es el que convierte Bayer en RGB. NO se toca CCM
         * (su matriz por defecto es de ceros y dejaria la imagen negra). */
        if (isp_video->gradient_ratio <= 0.0f) {
            isp_video->gradient_ratio = 2.0f;   /* valor neutro razonable */
        }
        if (isp_start_demosaic(isp_video) == ESP_OK) {
            ESP_LOGW(TAG, "PARCHE: demosaico ON");
        } else {
            ESP_LOGE(TAG, "PARCHE: demosaico FALLO");
        }
        {   /* Gamma neutra en los tres canales: deja pasar la senal tal cual. */
            isp_gamma_curve_points_t pts;
            esp_err_t g = esp_isp_gamma_fill_curve_points(joint_gamma_lineal, &pts);
            if (g == ESP_OK) {
                g |= esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_R, &pts);
                g |= esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_G, &pts);
                g |= esp_isp_gamma_configure(isp_video->isp_proc, COLOR_COMPONENT_B, &pts);
                g |= esp_isp_gamma_enable(isp_video->isp_proc);
                isp_video->gamma_started = true;
            }
            ESP_LOGW(TAG, "PARCHE: gamma neutra %s", g == ESP_OK ? "ON" : "FALLO");
        }
